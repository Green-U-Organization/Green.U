        ...
        <div className="h-screen max-w-screen">
            <div className="p-4 bg-white w-full max-w-md mx-auto mt-4">
                ...
                <div className="flex items-center">
                    ...
                    value={radius}
                    onChange={(e) => setRadius(Number(e.target.value))}
                    className="border rounded p-1 w-16 ml-2 mr-1"
                    />km
                </div>
            </div>
            <MapContainer center={position} zoom={13} style={{height: '100vh', width: '100%'}}>
                ...
                <Circle center={position} radius={radius * 1000} pathOptions={{ color: 'green', fillOpacity:0.1}}/>              
                ...
                {[...groupedLocations.entries()].map(([key, locs], index) => {
                    ...
                    <Marker key={index} position={[lat, lon]} icon={customIcon}>
                        <Popup>
                        <div>
                            <p>{translations.distance}: {calculateDistance(position, [lat, lon]).toFixed(1)} km</p>
                            <ul>
                                {locs.map((loc) => (
                                <li key={loc.code}>
                                    <Link href={`/landing/garden/`}>{loc.code}-{loc.city}</Link>
                                </li>
                                ))}
                            </ul>
                        </div>
